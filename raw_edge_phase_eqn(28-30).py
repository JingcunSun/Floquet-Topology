...
J_a = ((0.9 *2 * np.pi )/4)/ (T * 0.5) # sometimes I will twice this value here 
...
def real_H2N_fix_k_x(N, k_x, t): # t can be any from [  t_i = T/N * i + T/(N * 2)  ]
    ...
    for z in range (1, N+1):
        ...
        for i in range(N):
            for j in range(N):
                M[2*i:2*i+2, 2*j:2*j+2] += (1/N) * (np.exp(1j*  k_y * ((i+1) - (j+1)))) * H_ky
    return M
#%%
def H_list_6_matrices(k_x, k_y, N): # here this N referring to how many different stages (example: here is 6)
...
#Followed by: def real_H2N_fix_k_x(N, k_x, t): # t can be any from [  t_i = T/N * i + T/(N * 2)  ]

# this below function will produce a list of 6 H

def H_6_2N_edge(N, k_x): # here num of stages are fixed to be 6; lattice length a_0 fixed to be 1
    ...
    for x in range (6):
        ...
        real_H2N = real_H2N_fix_k_x(N, k_x, t_x)
        
        H2N_list_6_matrices.append(real_H2N) # generate 6 2N real H
        
        # from here, we are trying to turn the 2N matrix to edge condition
        real_H2N_edge = real_H2N. copy()
        rows, cols = real_H2N_edge .shape
...
# Set those entries to zero
        ...
        H2N_list_edge_6 . append(real_H2N_edge)
    
    return H2N_list_edge_6 # this will return the list of 6 edge functions which turned off corner already 

#%%
#continue by 2N by 2N for edges, the full time (expm) matrix for 2N by 2N dimension:
    
k_x_list_egde = np.linspace(0,  2 * np.pi, 100)
    
def U_full_ascending_edge(N, k_x): # which means along lhs to rhs, from (1) earlier/smaller t to (6) later/larger t
    
# here still, the num of stages are fixed as 6

    U_list_6_matrices =[]
    
    H_list_6_2N_edge = H_6_2N_edge(N, k_x)
    
    for i in range (6):
    
   #     t_i = (T/N) * i + T/(N * 2)
    
        H_i = H_list_6_2N_edge[i]
...
def plot_edge_vs_kx(k_x, N): # here k_x is a list 
    Ufull_kx_list =[]
    phase_list_of_list = []
#    phase2_T_asc_list = []
...
#        print(Ufull_matrix_asc_list)
    ...
    #    print(eigvals,'--eigen value;', eigvecs,'--eigen vector', i,'i') # all these eigen values are complex!!
        eigvals_kx_2N_list = eigvals_edge  # just rename
        
        print(eigvals_kx_2N_list, 'eigevals')
        
        phase_kx_2N_list = []
        
        for j in range (len(eigvals_kx_2N_list)):
            
            phase_kx_j = - np.angle(eigvals_kx_2N_list[j])
            
            phase_kx_2N_list. append (phase_kx_j) # this is list of phase for specific k_x, length 2N
       
        phase_list_of_list. append(phase_kx_2N_list) # this will be a list of different k_x, for each k_x has j
                                                     # -- layer one: len(k_x) -- layer two(each k_x), j_phases
        kx_list_for_plot. append([k_x[i]] * len(phase_kx_2N_list))
#          for kx_val in k_x:
...
#    plt.figure(figsize=(8, 5))
    plt.plot(kx_list_for_plot, phase_list_of_list, 'o', markersize=3, label='Edge phases')
...
#        phase_kx_2N_list. append (phase_kx_2N)
...
plot_edge_vs_kx(k_x_list_egde, 50)
...
M_1dSSH_4 = real_construction_1dSSH(4)
...
#def H_matrix_2by2(k_x, k_y, t, a_0)
              ...
              #  print(H_ky, 'H_ky')
                ...
                list_H_ky.append(H_ky)
                
               # print(z, 'z')
              #  H_ky = np.array([[1, 2], [3, 4]])
                
                M_2i_2j += (1/N) * (np.exp(1j*  k_y * ((i+1) - (j+1)))) * H_ky[0][0]
                
                M_2i_2j_1 += (1/N) * (np.exp(1j* k_y * ((i+1) - (j+1)))) * H_ky[0][1]
                
                M_2i_1_2j += (1/N) * (np.exp(1j* k_y * ((i+1) - (j+1)))) * H_ky[1][0]
                
                M_2i_1_2j_1 += (1/N) * (np.exp(1j* k_y * ((i+1) - (j+1)))) * H_ky[1][1]
...
# i is for row and j for column
 ...
 #           print (2*i, 2*i +1, 2*j, 2*j +1)
        print(list_z, 'list_z')
        print(list_k_y, 'list_k_y')
        print(list_H_ky, 'list_H_ky')
...
test_M_H1 = real_Hyy_fix_k_x(4, np.pi/3,  5/6 *T + T * 1/12)

#%%
# test the function to turn off 8 corner entrices

def turn_off_corner(real_H2N):
        # from here, we are trying to turn the 2N matrix to edge condition
...
test_M_H1_edge = turn_off_corner(test_M_H1)


# This matrix is when that only J_z is turned on (from 5/6 to 1, sixth stage)

# seems only when this J_z isn't turned off, there is the periodic term(which is corresponding to cos(x)) yes(?)

#%% check for k_basis matrix

def k_Hyy_fix_k_x(N, k_x, t): # t can be any from [  t_i = T/N * i + T/(N * 2)  ]
...
k_M_H1 = k_Hyy_fix_k_x(4, np.pi/3, T/3+T *1/12)
...
#   verify for the matrix before taken away the corner terms: try to diagonalise this test_M_H1:
   
eigvals_real2N, eigvecs_real2N = np.linalg.eig(test_M_H1)

k_eigvals_real2N, k_eigvecs_real2N = np.linalg.eig(k_M_H1)

print(eigvals_real2N)
print(k_eigvals_real2N)
...
#%% test - the function to diagonalise matrix

import numpy as np

# Generate a random 4x4 matrix
test_A = np.random.randint(1, 10, (4, 4))

print("4x4 Matrix:\n", test_A)

eigvals_test, eigvecs_test = np.linalg.eig(test_A)

print(eigvals_test)

# so the eigen value/ vector function is fine -- problem is : do our matrix fine? 
