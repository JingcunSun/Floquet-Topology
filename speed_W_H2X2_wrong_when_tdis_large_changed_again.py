...
def winding_number_enumerate(n_kx, n_ky, n_t, epsilon): # N_dis=20
    ...
    kx_list = np.linspace(0, 2*np.pi, n_kx, endpoint=False) #kx_list = np.linspace(0, 2*np.pi, n_kx, endpoint=True)
    ky_list = np.linspace(0, 2*np.pi, n_ky, endpoint=False)
    
    times_first_half   = np.linspace(0, T/2, (n_t )//2,  endpoint=False) # here only the first half time has discretization t
    times_second_half = np.linspace(T/2, T, (n_t )//2,  endpoint=False) # from the first point (small t)
                                              # later to check if endpoint should be true 
                                              # note: this is the grid sampled, cover end point will means: T--T+dt is sampled
                                              # to the second point (large t)
        ...
        for j, ky in enumerate(ky_list):
          ...
          #  U_t_list_from_0_to_halfT = [np.array([[1,0], [0,1]])] # help me double check if this is identity matrix
            ...
            for m, times1 in enumerate(times_first_half): # m from 0, corresponding t from small(0) to large(T/2)
                ...
                H_midpoint_time1_m = H_epsilon_H2by2_first(kx, ky, (times1 + (T/(2*n_t)))*2, epsilon) # **Calculate 
                cur_exp = expm(-1j *H_midpoint_time1_m * dt *2) # **Cal-exp, this is one exp for specific H with midpoint t
    ...
    for i, kx in enumerate(kx_list):
        ...
        for j, ky in enumerate(ky_list):
            for m, time in enumerate(times_first_half):
                ...
                u_inv_du_dt = np.matmul(u_inv_1[i, j, m], -1j* H_1[i, j, m]@u_1[i, j, m]* dt) 
