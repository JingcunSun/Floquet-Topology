...
def H2NyNx_xyOBC(N_y, N_x, t, a_0): 
    ...
    H_PBC_generated_both_OBC = H2NyNx_xOBC_only(N_y, N_x, t, a_0)

#    rows, cols =H_PBC_generated .shape #rows and cols are just vars - like rows = 15
    
    I, J = np.indices(H_PBC_generated_both_OBC.shape)
    ...
    H_PBC_generated_both_OBC [mask] = 0    
       # or whatever values you want
    print("\nMask (True means 'will be changed'):\n", mask)

    return H_PBC_generated_both_OBC
...
def find_spectrum_realy_OBC (N_y, k_x): # for calculating: spectrum_test_realy = Ufull_descending_PBC(N_y, k_x_TEST)
    Ufull_edge_kx_list =[]
    epsilon_n_list_of_kx_list = []
#    U_T = Ufull_descending_PBC(N_y, k_x)
    for i in range (len(k_x)): # This loop's each round run one specific k_x
...
spectrum_yOBC_NyNxCheck = find_spectrum_for_yOBC_check(N_y_TEST, N_x_TEST, a_0)

print('BEGIN: spectrum_test_realxy:', spectrum_test_realxy_version2, 'spectrum_test_realy', spectrum_test_realy, 'END')
#%%
...
if_sepctrum_are_the_same = same_values_ndp_yes_no(spectrum_test_realy_yOBC, spectrum_yOBC_NyNxCheck,5)
print('if_sepctrum_are_the_same', if_sepctrum_are_the_same)
