...
def U_epsilon_H2X2 (kx, ky, t, N_dis, epsilon): # N_dis stand for time discretisation we choose to calculate U(k,t) here 
    ...
    # alternative way record(little notebook p79): we can just not call any U function before, but classify t_epsilon into 6 stages, for each stage we have a certain parttern of 
        U_epsilon =  U_t_discretization(kx, ky, 6, t_epsilon)
...
UT_test_I = U_epsilon_H2X2 (np.pi*2, np.pi/3, T, 50, np.pi/15) #only_bulk_U_involved=True
print(UT_test_I)
...
def matrix_partial_dt(M, kx, ky, t, N_dis, epsilon, dt = 1e-6): # after M and before dt are just arg of M
                                                        # U = U_epsilon_H2X2 (kx, ky, t, N_dis, epsilon) 
...
#   which is one var ; multiple var: 
    dU_dt = matrix_partial_dt(U_epsilon_H2X2, kx, ky, t, N_dis, epsilon)
    dU_dkx = matrix_partial_dkx(U_epsilon_H2X2, kx, ky, t, N_dis, epsilon)
    dU_dky  = matrix_partial_dky(U_epsilon_H2X2, kx, ky, t, N_dis, epsilon)
...
W_res, W_err = quad3d(Tr_integrated, ( 0.0, 0.0, 0.0), (2*np.pi, 2*np.pi, 1), (50, np.pi))  # here epsilon = pi
...
# Q: do we need sympy to calculate the divU? The point is that each U_epsilon_H2X2 (kx, ky, t, N_dis, epsilon) 
