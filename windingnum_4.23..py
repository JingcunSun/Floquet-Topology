...
#    time_stage = num_time_stages
    ...
    for i in range (num_time_stages):
        ...
        t_i = t - t_discretization * (i + 1) #t_discretization = delta_t here 
...
def U_epsilon (kx, ky, t, N_dis, epsilon, only_bulk_U_involved): # N_dis stand for time discretisation we choose to calculate U(k,t) here 
                                 #only_bulk_U_involved will be =True or = False
                                 #Physically 'only_bulk_U_involved' means no time vortices 
    if only_bulk_U_involved:
        U_full_for_Heff = U_full_descending(kx, ky, num_time_stages = 6)#6 stages capture full U
    else: 
        
        U_full_for_Heff = U_full_discretisation(kx, ky, num_time_stages = N_dis) 
    # if we are clearly calculating bulk U here, we can use U_full_descending
...
UT_test_I = U_epsilon (np.pi*2, np.pi/3, T, 50, np.pi/15, only_bulk_U_involved=True)
print(UT_test_I)
